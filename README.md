...
这是一个个人自用的 Windows 原生桌面版本，用 WPF 显示 Codex 的一周额度和刷新时间状态。5 小时额度窗口暂停期间，组件不会显示或依据它调整刷新频率。窗口默认无边框、置顶、贴靠任务栏左侧，并提供托盘菜单。
...
- `WK` 使用圆形向量 gauge，以绿色、黄色、红色提示余量状态；阈值可在 `settings.json` 中调整。
...
额度查询通过启动本机 `codex.exe app-server --listen stdio://`，再以 JSON-RPC 调用 `account/rateLimits/read` 读取 `windowDurationMins` 为 `10080` 的一周额度窗口。Codex 可能将该窗口放在 `primary` 或 `secondary` 字段；5 小时窗口会被忽略。本项目只是读取本机 Codex 程序返回的数据，不提供官方额度接口，也不是 OpenAI 官方工具。
